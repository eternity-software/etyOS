...
if [ "${YAWC_ALLOW_DIRECT_TTY_RUN:-0}" != "1" ]; then
  echo "error: direct tty backend launch is disabled by default" >&2
  echo "hint: use ./scripts/start-tty.sh so YAWC starts on a dedicated VT" >&2
  echo "hint: set YAWC_ALLOW_DIRECT_TTY_RUN=1 only if you intentionally want the unsafe direct path" >&2
  exit 1
fi
...
if [ -x "$SYSROOT_ROOT/usr/bin/pkg-config" ]; then
  ...
  echo "error: pkg-config is required for the experimental tty-udev backend" >&2
  echo "hint: either install system packages or run ./scripts/bootstrap-tty-deps.sh" >&2
  exit 1
fi
...
if [ -n "$PKG_CONFIG_LIBDIR" ]; then
  exec env \
    ...
    "$YAWC_CARGO_BIN" run --no-default-features --features tty-udev -- --tty-udev "$@"
fi
