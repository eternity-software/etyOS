...
SCRIPT_PATH="$SCRIPT_DIR/start-tty.sh"
LOG_DIR="$ROOT_DIR/logs"
LOG_FILE="${YAWC_TTY_LOG:-$LOG_DIR/tty-session.log}"
VT_NUMBER="${YAWC_TTY_VT:-8}"
LOCAL_LIB_DIR="$ROOT_DIR/.local-lib"
SYSROOT_ROOT="$ROOT_DIR/.sysroot/root"
BINARY_PATH="$ROOT_DIR/target/debug/yawc"
LAUNCHER_PATH="$ROOT_DIR/scripts/launch-tty-binary.sh"
...
echo "YAWC tty launch log: $LOG_FILE"
...
if [ "${YAWC_SKIP_TTY_BUILD:-0}" != "1" ]; then
  echo "Building YAWC tty binary before VT handoff..." >&2
...
if [ "${YAWC_USE_OPENVT:-0}" != "1" ]; then
  if [ "$(id -u)" -eq 0 ]; then
    echo "error: do not run the default tty launcher with sudo/root" >&2
    echo "hint: switch to a real TTY, log in as $RUN_AS_USER, then run ./scripts/start-tty.sh" >&2
  ...
  case "$current_tty" in
    /dev/tty[0-9]*)
      ;;
    *)
      echo "error: start-tty.sh must be run from a real Linux TTY, e.g. Ctrl+Alt+F3" >&2
      echo "hint: current terminal is '$current_tty', not /dev/ttyN" >&2
      exit 1
      ;;
  esac

  echo "Launching YAWC on current TTY as ${RUN_AS_USER}..." >&2
