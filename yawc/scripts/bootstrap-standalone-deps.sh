...
if ! command -v apt-get >/dev/null 2>&1; then
  echo "error: apt-get is required to bootstrap local standalone dependencies" >&2
  exit 1
fi

if ! command -v dpkg-deb >/dev/null 2>&1; then
  echo "error: dpkg-deb is required to unpack local standalone dependencies" >&2
  exit 1
fi
...
echo "bootstrapped local standalone sysroot in $SYSROOT_DIR"
